...
class Home {
    ...
    async toggleVanillaDropdown() {
        let vanillaDropdown = document.querySelector('.vanilla-dropdown')
        let vanillaArrow = document.querySelector('.vanilla-arrow')
        
        if (vanillaDropdown.style.display === 'none') {
            // Open dropdown
            vanillaDropdown.style.display = 'block'
            vanillaArrow.style.transform = 'rotate(180deg)'
    ...
    closeVanillaDropdown() {
        let vanillaDropdown = document.querySelector('.vanilla-dropdown')
        let vanillaArrow = document.querySelector('.vanilla-arrow')
        
        vanillaDropdown.style.display = 'none'
        vanillaArrow.style.transform = 'rotate(0deg)'
    }

    async loadMinecraftVersions() {
        try {
            const vanillaVersionsList = document.querySelector('.vanilla-versions-list')
            vanillaVersionsList.innerHTML = '<div class="loading-versions">Chargement des versions...</div>'
            ...
            const vanillaVersionsList = document.querySelector('.vanilla-versions-list')
            vanillaVersionsList.innerHTML = '<div class="loading-versions">Erreur de chargement des versions</div>'
        }
    }

    async selectMinecraftVersion(versionId, versionType) {
        // Close dropdown
        this.closeVanillaDropdown()
    ...
    async startVanillaGameWithVersion(selectedVersion = 'release') {
        ...
        let opt = {
            ...
            instance: `vanilla-${selectedVersion}`,
        ...
        launch.on('progress', (progress, size) => {
            infoStarting.innerHTML = `Téléchargement ${selectedVersion} ${((progress / size) * 100).toFixed(0)}%`
        ...
        launch.on('check', (progress, size) => {
            infoStarting.innerHTML = `Vérification ${selectedVersion} ${((progress / size) * 100).toFixed(0)}%`
            ipcRenderer.send('main-window-progress', { progress, size })
            progressBar.value = progress;
            progressBar.max = size;
        });
        ...
        launch.on('patch', patch => {
            console.log(patch);
            ipcRenderer.send('main-window-progress-load')
            infoStarting.innerHTML = `Patch ${selectedVersion} en cours...`
        });
        ...
        launch.on('data', (e) => {
            ...
            new logger(`Minecraft ${selectedVersion}`, '#36b030');
            ipcRenderer.send('main-window-progress-load')
            infoStarting.innerHTML = `Démarrage ${selectedVersion} en cours...`
            console.log(e);
        })
    ...
    async instancesSelect() {
        ...
        let vanillaBTN = document.querySelector('.vanilla-launcher')
        let vanillaPopup = document.querySelector('.vanilla-popup')
        let vanillaVersionsList = document.querySelector('.vanilla-versions-list')
        ...
        vanillaBTN.addEventListener('click', async (e) => {
            ...
            vanillaPopup.style.display = 'flex'
        })
        
        vanillaPopup.addEventListener('click', async (e) => {
            if (e.target.classList.contains('version-item')) {
                let selectedVersion = e.target.dataset.version
                let selectedType = e.target.dataset.type
                
                // Remove active class from all versions
                document.querySelectorAll('.version-item').forEach(item => {
                    item.classList.remove('active-version')
                })
        ...
        vanillaCloseBTN.addEventListener('click', () => {
            vanillaPopup.style.display = 'none'
        })
